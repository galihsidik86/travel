-- Stage 213 — add CREW_DIETARY_BRIEF notif type. Mon morning email
-- per crew on a paket departing within `windowDays`. Lists every
-- non-REGULAR jemaah on the manifest so muthawwif can brief the
-- hotel kitchen on arrival.
