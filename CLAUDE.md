...
| send-daily-digest (S27) | `npm run job:send-daily-digest` | `POST /api/admin/jobs/send-daily-digest` | `buildDailyDigest` + `notifyDailyDigest` | Build yesterday's activity summary + enqueue one EMAIL per ACTIVE OWNER |
